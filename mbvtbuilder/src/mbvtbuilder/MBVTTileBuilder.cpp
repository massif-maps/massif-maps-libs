...
namespace carto::mbvtbuilder
{
    ...
    void MBVTTileBuilder::addMultiPoint(LayerIndex layerIndex, MultiPoint coords, picojson::value id, picojson::value properties, const bool updateOnly)
    {
        ...
        uint64_t actualId = extractFeatureId(layerIndex, id, properties);
        if (updateOnly) {
            auto it = std::remove_if(layer.features.begin(), layer.features.end(), [&actualId](const Feature& feature) -> bool
            {
                return feature.id == actualId;
            });
            if (it != layer.features.end())
            {
                layer.features.erase(it);
                // TODO: we dont update layer bounds. The only way right not is to compute it all again with all remaining features
            } else {
                return;
            }
        }
    ...
    void MBVTTileBuilder::addMultiPolygon(LayerIndex layerIndex, MultiPolygon ringsList, picojson::value id, picojson::value properties, const bool updateOnly)
    {
        ...
        layer.bounds.add(feature.bounds);
    ...
    void MBVTTileBuilder::removeGeoJSONFeature(LayerIndex layerIndex, const std::uint64_t id)
    {
        std::lock_guard<std::mutex> lock(_mutex);
        Layer &layer = _layers[layerIndex];
        auto it = std::remove_if(layer.features.begin(), layer.features.end(), [&id](const Feature& feature) -> bool
        {
