...
using Expression = std::variant<
    Value, FieldOrVar, std::shared_ptr<StringExpression>,
    std::shared_ptr<ListExpression>, std::shared_ptr<UnaryExpression>,
    std::shared_ptr<BinaryExpression>, std::shared_ptr<ConditionalExpression>,
...
public:
  explicit ListExpression(std::vector<Expression> exprs)
      : _exprs(std::move(exprs)) {}

  const std::vector<Expression> &getExpressions() const { return _exprs; }

  bool operator==(const ListExpression &other) const {
    return _exprs == other._exprs;
  }
  bool operator!=(const ListExpression &other) const {
    return !(*this == other);
  }
...
public:
  ...
  explicit UnaryExpression(Op op, Expression expr)
...
public:
  ...
  explicit BinaryExpression(Op op, Expression expr1, Expression expr2)
  ...
  bool operator==(const BinaryExpression &other) const {
    return _op == other._op && _expr1 == other._expr1 && _expr2 == other._expr2;
  }
  bool operator!=(const BinaryExpression &other) const {
    return !(*this == other);
  }
...
public:
  explicit ConditionalExpression(Expression cond, Expression expr1,
                                 Expression expr2)
      : _cond(std::move(cond)), _expr1(std::move(expr1)),
  ...
  bool operator==(const ConditionalExpression &other) const {
    return _cond == other._cond && _expr1 == other._expr1 &&
           _expr2 == other._expr2;
  }
  bool operator!=(const ConditionalExpression &other) const {
    return !(*this == other);
...
public:
  explicit FunctionExpression(std::string func, std::vector<Expression> args)
  ...
  bool operator==(const FunctionExpression &other) const {
    return _func == other._func && _args == other._args;
  }
  bool operator!=(const FunctionExpression &other) const {
    return !(*this == other);
  }

  static Value applyFunc(const std::string &func,
