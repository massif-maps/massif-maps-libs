...
namespace carto::mvt {
    TextSymbolizer::FeatureProcessor TextSymbolizer::createFeatureProcessor(const ExpressionContext& exprContext, const SymbolizerContext& symbolizerContext, const std::shared_ptr<const Rule>& rule) const {
        ...
        // Get allow-clustering from this text
        bool allowClustering = _allowClustering.getValue(exprContext);
        
        // Get cluster-distance from ClusterSymbolizer if present in rule
                    ...
                    for (const auto& vertices : verticesList) {
                        for (const auto &vertex: vertices) {
                            textProcessor(localId, 10 * labelId + index, groupId, vertex,
                                          ...
                                          sameFeatureIdDependent, index, allowClustering, clusterDistance, clusterGroupId);
                        }
                        index++;
                    }
                }
                    ...
                    for (const auto& vertices : verticesList) {
                        ...
                        for (const auto& transformedPoints : transformedPointsList) {
                            ...
                            for (const auto& vertex : transformedPoints.second) {
                                ...
                                textProcessor(localId, generatedLabelId, groupId, vertex, vt::TileLayerBuilder::Vertices(), text, placementPriority, minimumDistance, allowOverlapSameFeatureId, sameFeatureIdDependent, 0, allowClustering, clusterDistance, clusterGroupId);
                                counter++;
                            }
                        }
                    }
